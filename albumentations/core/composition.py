...
class BatchCompose(Compose):
    ...
    Note this naming rule is applied to the `label_fields` of the `BboxParams` and the `KeypointsParams`.
...
class ForEach(BaseCompose):
    ...
    This can contain any other transforms but a subclass of the `BatchBasedTransform`.
...
class Repeat(BaseCompose):
    ...
    This expects batched input and can be contained by the `BatchCompose`.
    The contained transforms should be a subclass of the `BatchBasedTransform`.
    ...
    def __call__(self, *args, force_apply: bool = False, **batched_data) -> typing.Dict[str, typing.List]:
        processed = []
        ori_image = batched_data["image_batch"][0].copy()
        for _ in range(self.n):
            data = batched_data
            for t in self.transforms:
                data = t(force_apply=force_apply, **data)
            processed.append(data)
            assert np.all(batched_data["image_batch"][0] == ori_image), "Image should not be altered after transforms."
